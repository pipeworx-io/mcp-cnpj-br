...
```json
{
  "mcpServers": {
    "cnpj-br": {
      "url": "https://gateway.pipeworx.io/cnpj-br/mcp"
    }
  }
}
```
...
`tools/list` at `https://gateway.pipeworx.io/cnpj-br/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/cnpj_lookup \
  -H 'Content-Type: application/json' \
  -d '{"cnpj":"33683111000280"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/cnpj_lookup`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```
ask_pipeworx({ question: "your question about Cnpj Br data" })
```
