...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'CNPJ Brazil');
}
...
async function cnpjLookup(args: Record<string, unknown>) {
  ...
  const res = await pwFetch(`${BASE_URL}/${cnpj}`, { headers: { Accept: 'application/json' } });
  ...
  if (res.status === 404) {
    throw new Error(
      `CNPJ ${formatCnpj(cnpj)} was not found in the Receita Federal registry. The checksum is ${validateChecksum(cnpj).valid ? 'valid, so this is either an unregistered number or a registration newer than the latest monthly data dump' : 'also invalid — likely a typo'}.`,
    );
  }
  ...
  if (!res.ok) {
    throw await httpError(res, 'cnpj_lookup: minhareceita.org error');
  }
